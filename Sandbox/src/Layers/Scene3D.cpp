	...
	m_Vfs("C:\\Users\\Artsiom\\Documents\\Projects\\CraftEngine\\Sandbox\\src\\Shaders\\")
{
	...
	m_Shader->Link();
	m_Shader->Use();
	m_Shader->SetUniform3f("Kd", v3(0.9f, 0.5f, 0.3f));
	m_Shader->SetUniform3f("Ld", v3(1.0f, 1.0f, 1.0f));
	m_Shader->SetUniform4f("LightPosition", v4(5.0f, -5.0f, 2.0f, 1.0f));
	m_Shader->Unuse();
...
void Scene3D::OnUpdate(f32 deltaTime) {
	rotate = sinf(deltaTime) * 50;

	model *= mat4::Rotate(rotate, v3(0.0f, 1.0f, 0.0f));
}
